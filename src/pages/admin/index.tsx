...
const AutoDiscoverySection = ({
  ...
  const generateCommand = () => {
    ...
    let scriptUrl = `https://raw.githubusercontent.com/bi11zh3nn/komari-agent/refs/heads/main/${scriptFile}`;
    if (enableGhproxy && ghproxy) {
    ...
    switch (selectedPlatform) {
      ...
      case "docker": {
        ...
        finalCommand =
          ...
          `ghcr.io/bi11zh3nn/komari-agent:latest ` +
...
function GenerateCommandButton({
  ...
  const generateCommand = () => {
    ...
    let scriptUrl =
      `https://raw.githubusercontent.com/bi11zh3nn/komari-agent/refs/heads/main/${scriptFile}`;
    if (enableGhproxy) {
      if (enableGhproxy && ghproxy) {
        scriptUrl = scriptUrl.slice(8); // 去掉 https://
        if (ghproxy.endsWith("/")) {
